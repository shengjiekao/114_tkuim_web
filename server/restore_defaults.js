...
const defaultEvents = [
    {
        ...
        date: new Date(date.getTime() + 7 * 24 * 60 * 60 * 1000), // 1 week later
    ...
    },
    {
        ...
        date: new Date(date.getTime() + 14 * 24 * 60 * 60 * 1000), // 2 weeks later
    ...
    },
    {
        ...
        date: new Date(date.getTime() + 21 * 24 * 60 * 60 * 1000), // 3 weeks later
...
async function restoreDefaults() {
    try {
        ...
        for (const evt of defaultEvents) {
            ...
            if (!exists) {
                console.log(`Restoring missing event: ${evt.title}`);
                // Re-creating event. Note: if Soft Delete was active in schema, we'd enable it, but currently schema doesn't have isDeleted (we reverted it).
                // So we just insert.
                await new Event(evt).save();
            } else {
